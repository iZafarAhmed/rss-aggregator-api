...
app.get('/api/news', async (req, res) => {
  ...
  try {
    ...
    if (keyword) {
      ...
      if (terms.length > 0) {
        filtered = filtered.filter(item => terms.some(term => (item.title && item.title.toLowerCase().includes(term)) || (item.description && item.description.toLowerCase().includes(term))));
      }
    }
