...
async function fetchFeed(source, limit = 4) {
  try {
    ...
    return (feed.items || []).slice(0, limit).map(item => {
      ...
      // 1. <enclosure type="image/..."> (common in WordPress, CBC, etc.)
      if (item.enclosure?.url && item.enclosure?.type?.startsWith('image/')) {
        imageUrl = item.enclosure.url;
      }
      // 2. <media:content medium="image" /> or type="image/..."
      else if (item['media:content']) {
        const media = item['media:content'];
        // media can be object or array — handle both
      ...
      // 3. Fallback: extract first <img src> from description
